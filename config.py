...
def redirect_request_to_region(request_method:str, url:str, region_code:str, current_region:str, payload:dict = None):
    print('Data not found in current region, re-directing.', flush=True)
    new_api_port = config[region_code]['api_port']
    new_url = url.replace(current_region.lower(), 'nginx-' + region_code.lower() + ':' + str(new_api_port))
    print("Redirecting URL:", flush=True)
